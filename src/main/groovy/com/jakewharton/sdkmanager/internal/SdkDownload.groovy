...
enum SdkDownload {
    ...
    void download(File dest, String SDK_VERSION) {
        ...
        temp.withOutputStream {
            def proxyHost = java.lang.System.getProperty('http.proxyHost');
            def proxyPort = java.lang.System.getProperty('http.proxyPort');
            if (proxyHost != null && proxyPort != null) {
                log.debug "Use proxy to downaload SDK: $proxyHost:$proxyPort"
                def proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(proxyHost, proxyPort.toInteger()))
                it << new URL(url).openConnection(proxy).content
            } else {
