...
class SdkResolver {
    ...
    def downloadSdk(File target) {
        String SDK_VERSION
        if (project.hasProperty('SDK_VERSION')) {
            SDK_VERSION = project.getProperties().get('SDK_VERSION')
        } else {
            throw new StopExecutionException('Project do not provide SDK_VERSION property. Could not download SDK')
        }
        log.lifecycle "Android SDK not found. Downloading version $SDK_VERSION..."


        // Download the SDK zip and extract it.
        downloader.download(target, SDK_VERSION)
